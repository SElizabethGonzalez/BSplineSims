...
xpoints = [-1.3773773660399793, -1.1544851789147135, -0.5572598316772178, -0.16129949752779843, -0.7109562528294372, -1.2511382563859468, -0.8395595134447649, -0.24867497156474128, 0.0]
ypoints = [-0.766532305659816, -0.7009622526263379, -0.5692057328056023, -0.018614889953004473, 0.920723927896786, 1.8611791860974576, 2.4077975649647145, 2.5268800332967816, 2.5959465655318184]
zpoints= [0.0, -0.042855282134685646, -0.1259525042371836, -0.7383569249902951, -1.1344907224612821, -0.7296002571025396, -0.12011340936653692, -0.012539551054758971, 0.056497148458322495]



# scale the x control points so it matches the cell width
# prevents overlapping with the compression fnc
xpoints = fixwidth(xpoints, initialwidth/2)
...
function gradientdescent(cpt1, cpt2, cpt3, learn_rate, conv_threshold, max_iter)
    ...
    oldenergy = totalenergy(cpt1, cpt2, cpt3, height, initialwidth)
    ...
    width = 2*(cpt1[9] - cpt1[1])

    totalenergy(cpt1, cpt2, cpt3, height, width, true)


    println("Here is the width:")
    println(width)
...
grad = gradientdescent(xpoints, ypoints, zpoints, 0.0001, 0.000000001, 100000)
